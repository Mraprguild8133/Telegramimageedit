...
Ready to edit your photos! 📸
    ...
    async def process_resize(self, query, photo_path, action):
        ...
        try:
            ...
            if not hasattr(self.image_processor, 'resize_image'):
                await query.edit_message_text("❌ Resize feature is not available.")
                return
            
            result_path = self.image_processor.resize_image(photo_path, width, height)
            
            if not result_path or not os.path.exists(result_path):
                await query.edit_message_text("❌ Failed to resize image.")
                return
            ...
            if result_path != photo_path and result_path not in self.temp_files:
                try:
    ...
    async def process_crop(self, query, photo_path, action):
        ...
        try:
            ...
            # Check if crop method exists
            if not hasattr(self.image_processor, 'smart_crop'):
                await query.edit_message_text("❌ Crop feature is not available.")
                return
            ...
            if not result_path or not os.path.exists(result_path):
                await query.edit_message_text("❌ Failed to crop image.")
                return
            ...
            with open(result_path, 'rb') as photo:
                await query.message.reply_photo(
                    photo=photo,
                    caption=f"✅ *Image cropped ({crop_mode.title()})*",
                    parse_mode='Markdown'
                )
    ...
    async def process_remove_background(self, query, photo_path):
        ...
        try:
            ...
            # Check if remove_background method exists
            if not hasattr(self.image_processor, 'remove_background'):
                await query.edit_message_text("❌ Background removal feature is not available.")
                return
            
            result_path = self.image_processor.remove_background(photo_path)
            
            if not result_path or not os.path.exists(result_path):
                await query.edit_message_text("❌ Failed to remove background.")
                return
    ...
    async def process_generate_background(self, query, photo_path):
        ...
        try:
            ...
            # Check if generate_background method exists
            if not hasattr(self.image_processor, 'generate_background'):
                await query.edit_message_text("❌ Background generation feature is not available.")
                return
            
            result_path = self.image_processor.generate_background(photo_path)
            
            if not result_path or not os.path.exists(result_path):
                await query.edit_message_text("❌ Failed to generate background.")
                return
    ...
    async def process_enhance_quality(self, query, photo_path):
        ...
        try:
            ...
            # Check if enhance_quality method exists
            if not hasattr(self.image_processor, 'enhance_quality'):
                await query.edit_message_text("❌ Quality enhancement feature is not available.")
                return
            
            result_path = self.image_processor.enhance_quality(photo_path)
            
            if not result_path or not os.path.exists(result_path):
                await query.edit_message_text("❌ Failed to enhance quality.")
                return
    ...
    async def process_convert_format(self, query, photo_path, action):
        ...
        try:
            ...
            # Check if convert_format method exists
            if not hasattr(self.image_processor, 'convert_format'):
                await query.edit_message_text("❌ Format conversion feature is not available.")
                return
            
            result_path = self.image_processor.convert_format(photo_path, format_name.lower())
            
            if not result_path or not os.path.exists(result_path):
                await query.edit_message_text("❌ Failed to convert format.")
                return
